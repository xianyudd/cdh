...
排除清单是 `$XDG_DATA_HOME/cdh/excludes`（默认 `~/.local/share/cdh/excludes`），每行一个绝对路径，`#` 开头为注释，可以手工编辑；文件不存在等于空清单。它和 `CDH_IGNORE_RE` 的分工是：正则面向「在脚本或配置里预先声明」，清单面向「用的时候顺手清掉眼前的噪音」，两者都同时作用于历史候选和发现候选。
