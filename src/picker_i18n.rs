...
pub(super) enum TextKey {
    ...
    OpenExcludes,
    HiddenDirectoriesFiltered,
    HiddenDirectoriesShown,
    NoDeletableHistory,
    MissingDeleteHint,
    NoJumpTarget,
    TerminalTooSmall,
    ...
    pub(super) fn text(self, key: TextKey) -> &'static str {
        ...
        match key {
            ...
            FooterCompact => self.pick(
                "↑↓ 选择 · Ctrl+H 隐藏目录 · Enter 跳转 · Tab 预览 · F1 帮助 · F2 设置 · Esc 退出",
                "↑↓ Select · Ctrl+H Hidden dirs · Enter Jump · Tab Preview · F1 Help · F2 Settings · Esc Exit",
            ),
            FooterShort => self.pick(
                "Ctrl+H 隐藏目录 · Enter 跳转 · F1 帮助 · F2 设置 · Esc 退出",
            ...
            HistoryDeleted => self.pick(
                "已排除该目录及其子目录",
                "Directory and everything under it excluded",
            ),
            PreviewUnavailable => self.pick("预览功能不可用", "Preview unavailable"),
            HistoryWriteUnavailable => self.pick(
                "当前环境无法写入 cdh 数据文件",
                "cdh data files cannot be written in this environment",
            ),
            ...
            HiddenDirectoriesFiltered => self.pick("已过滤隐藏目录", "Hidden directories filtered"),
            HiddenDirectoriesShown => self.pick("已显示隐藏目录", "Hidden directories shown"),
            NoDeletableHistory => self.pick("没有可排除的目录", "No directory to exclude"),
            MissingDeleteHint => self.pick(
                "目录已失效，按 Ctrl+D 排除它",
                "Directory is missing; press Ctrl+D to exclude it",
            ),
            ...
            DeleteHistoryEntry => self.pick(
                "排除当前目录及其子目录",
                "Exclude directory and its subtree",
            ),
            ToggleHiddenDirectories => self.pick("切换隐藏目录过滤", "Toggle hidden directory filter"),
            ...
            ConfirmDeleteTitle => self.pick("确认排除", "Confirm exclusion"),
            ConfirmDeleteAgain => self.pick(
                "再次按 Ctrl+D 确认，Esc 取消",
                "Press Ctrl+D again to confirm, Esc to cancel",
            ),
            // 「及其子目录」不能省：排除是子树语义，用户按下去消失的可能是几千行。
            ConfirmDeletePrefix => self.pick("不再显示 “", "Stop showing “"),
            ConfirmDeleteSuffix => self.pick("” 及其子目录？", "” and everything under it?"),
            ConfirmDeleteShort => self.pick("确认排除该目录及其子目录", "Confirm subtree exclusion"),
